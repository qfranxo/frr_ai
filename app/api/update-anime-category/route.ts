...
/**
 * 이미지 카테고리 업데이트 API
 * 프롬프트에 'anime', 'animation', 'cartoon' 등이 들어있는 이미지를 찾아 
 * 카테고리를 'anime'로 업데이트
 */
export async function GET() {
  try {
    // 'anime' 관련 키워드를 포함하는 이미지 검색 (프롬프트 기반)
    const { data: images, error: searchError } = await supabase
      .from('shared_images')
      .select('id, prompt, rendering_style, category')
      .or('prompt.ilike.%anime%,prompt.ilike.%animation%,prompt.ilike.%cartoon%,prompt.ilike.%manga%,rendering_style.ilike.%anime%,rendering_style.ilike.%animation%,rendering_style.ilike.%cartoon%')
    ...
    // 각 이미지의 카테고리를 'anime'로 업데이트
    const updateResults = [];
    
    for (const image of images) {
      // anime 관련 키워드가 있으면 무조건 anime로 설정
      let newCategory = 'anime'; // 기본값으로 anime 설정

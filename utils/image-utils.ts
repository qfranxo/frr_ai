...
export function isValidImageUrl(imageUrl: string | null | undefined): boolean {
  if (!imageUrl) return false;
  if (imageUrl.trim().length === 0) return false;
...
export function saveReplicateUrlToStorage(
  ...
  // 로그 추가
  console.log(`[저장 시작] ID: ${postId}, URL: ${imageUrl.substring(0, 30)}..., 타입: ${type}`);
  
  // 이미 처리 중이거나 Replicate URL이 아니면 처리하지 않음
  if (processingUrls.has(imageUrl) || !isReplicateUrl(imageUrl)) {
    console.log(`[저장 중단] ID: ${postId}, 이유: ${processingUrls.has(imageUrl) ? '이미 처리 중' : 'Replicate URL 아님'}`);
    return;
  }
  ...
  setTimeout(() => {
    ...
    fetch('/api/image-storage', {
      ...
      .then(data => {
        if (data?.success) {
          console.log(`[저장 성공] ID: ${postId}`);
        } else {
          console.warn(`[저장 실패] ID: ${postId}, 오류: ${data?.error || '알 수 없음'}`);
        }
      })
      .catch(error => {
        console.error(`[심각] 저장 오류 ID: ${postId}`, error.message || error);
